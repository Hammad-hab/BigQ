...
function Question({ question, answer, index }) {
  ...
  return (
    <div className={
        selected !== false ? 
        "alert alert-dark selected":
        "alert alert-dark"
        } id={index}>
          <small className="badge me-auto cwtitle" style={{"color": status == "saved" ? "green" : "red"}} title={
            status === "saved"? "All changes are saved, if you reload the tab no data will be lost." : "Data is not saved. If you reload the tab all changes will be lost."
          }>{status}</small>
      <span id={"s-" + index} onDoubleClick={() => select(!selected)}>
        ...
        <p>
          ...
          <span
            ...
            onSelect={(e) => {
              ...
              const prvQ = document.getElementById("q-"+index).getAttribute("data-Qprev")
              const prvA = document.getElementById("a-"+index).getAttribute("data-Aprev")
        ...
        <p>
          Answer: <span contentEditable id={"a-" + index} data-Aprev={answer}
          onSelect={(e) => {
            const NewQ = document.getElementById("q-"+index).innerText
            const NewA = document.getElementById("a-"+index).innerText
      ...
      <RiDeleteBin2Line
        ...
        onClick={() => {
          All.getDataAsString(
            `remove?q=${question}&a=${answer}`
          ).then((r) => {
            document.getElementById(String(index)).remove();
          });
        }}
      />
      <FaSave className="widget" title={"save all edits"} onClick={() => {
