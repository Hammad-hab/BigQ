  ...
  return (
    <div className="toolbar">
      ...
      <Offcanvas heading={"New question"} name="nq">
        <>
          <raw>Question : </raw>
          <input id="ques" onChange={() => document.getElementById("ques").value.trim().length > 0 ? setRule(false) : setRule(true)}/>
          <br />
          <br />
          <raw>Options: </raw>
          <div id="container">
          ...
          <button 
            disabled={disable}
            className={!disable ? "trn-all btn btn-primary" : "trn-all btn btn-danger"}
            onClick={() => {
              All.getDataAsString(
                `submitData?q=${
                  document.getElementById("ques").value
